...
#   1. `pip install -e .` (llama-cpp-python is a standard dependency, not an
#      extra - the historical `[llm]` extra is now a no-op kept only for
#      compatibility with older instructions). This step compiles
#      llama-cpp-python's native backend from source against this machine's
#      CPU/GPU (Metal on Apple Silicon), which can take several minutes on
#      first install - that is expected, not a hang.
...
# llama-cpp-python check: it's a required dependency, so if it's missing
# here the native build most likely failed silently earlier (e.g. a
# missing CMake/compiler toolchain) rather than someone having opted out
# of an extra - only the rule-based backend will ship if this fires.
HAS_LLAMA=0
if python3 -c "import llama_cpp" 2>/dev/null; then
    ...
    echo "even though it is a required dependency. The build will continue,"
    echo "but only the 'rule' backend will function - the native compile"
    echo "likely failed earlier. Re-run 'pip install -e .' and check for"
    echo "CMake/compiler errors above, then re-run this script."
    echo ""
fi
...
python3 -m PyInstaller "${PYINSTALLER_ARGS[@]}" "$PROJECT_ROOT/src/promptsmith/cli/launcher.py"
...
Project home: https://github.com/pr0z4k/promptsmith-cli
Support:      https://github.com/pr0z4k/promptsmith-cli/issues
NOTEEOF
fi
