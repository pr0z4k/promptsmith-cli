...
`llama-cpp-python` has no prebuilt wheel on PyPI: installing it compiles
its native `llama.cpp` backend from source (CMake + a C/C++ toolchain),
tuned against the machine doing the build. On first install this
typically takes several minutes - longer on older hardware or a cold
compiler cache - and is normal, not a hang. Compiling from source (rather
than a generic prebuilt wheel) is deliberate: on Apple Silicon it enables
Metal GPU acceleration, which meaningfully changes local-model inference
speed.
...
python -m pip install -c requirements/constraints-release.txt -e ".[build]"
```
...
```powershell
python -m pip install -c requirements/constraints-release.txt llama-cpp-python --extra-index-url https://abetlen.github.io/llama-cpp-python/whl/cpu
```
