...
def _validate_checksum(expected_sha256: str | None) -> str | None:
...
def _stream_download(
    ...
    if dest_path.parent.is_symlink():
        raise DownloadSecurityError(
            f"Refusing to write through symlinked directory: {dest_path.parent}"
        )
    ...
    try:
        ...
        raise RuntimeError(
            "Missing dependency: requests. Install project download dependencies"
        ) from exc
    ...
    for attempt in range(1, max_attempts + 1):
        ...
        try:
            ...
            if not is_valid_gguf(part_path):
                raise InvalidModelFileError(
                    f"{label}: downloaded content is not a valid GGUF model"
                )
...
def main(progress_callback: ProgressCallback | None = None) -> dict[str, Any]:
    logging.basicConfig(
        level=logging.INFO,
