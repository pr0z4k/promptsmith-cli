...
from .prompt_analyzer import PromptAnalysis, PromptAnalyzer, PromptRecommendation
...
class IntentCompiler:
    ...
    def compile(
        ...
        try:
            ...
            refined_prompt = self.refiner.refine(
                working_prompt, profile_name=final_profile, template_name=template_name
            )
        ...
        logger.info(
            f"Intent compilation complete. Score: {compilation_score}/100, Ready: {is_ready}"
        )
        return result
    ...
    def compile_and_refine(
        ...
        """
        result = self.compile(prompt, profile_name=profile_name, template_name=template_name)
        return result.refined_prompt

    def get_analysis_only(self, prompt: str, profile_name: str | None = None) -> PromptAnalysis:
        """
    ...
    def _calculate_compilation_score(self, analysis: PromptAnalysis, was_refined: bool) -> int:
        """
...
def compile_prompt(
    ...
    return compiler.compile(prompt, profile_name=profile_name, template_name=template_name)
