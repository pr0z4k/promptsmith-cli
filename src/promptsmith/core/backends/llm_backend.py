...
class LLMBasedBackend(ModelBackend):
    ...
    def _load_model(self) -> bool:
        with self._lock:
            ...
            try:
                ...
                self.last_error = (
                    "llama-cpp-python is unavailable or failed to import. "
                    "Reinstall PromptSmith, or install the compatible "
                    "llama-cpp-python wheel for your platform (see BUILD.md)."
                )
    ...
    @staticmethod
    def _validate_chat_format(name: str | None) -> str | None:
        if name is None:
            return None
        try:
    ...
    def _detect_chat_format(self) -> str | None:
        if self.model_path is None:
            return None
