...
class PromptAnalyzer:
    ...
    TYPE_PATTERNS: dict[str, tuple[list[str], str, str]] = {
        ...
        "email": (
            ["email", "letter", "write to", "respond to", "compose", "draft", "subject line"],
            "email",
            "General",
        ),
        "executive": (
        ...
        "research": (
            ["research", "investigate", "compare", "evaluate", "analysis", "look into", "explore"],
            "research",
            "Research Analyst",
        ),
    }
    ...
    AMBIGUOUS_TERMS: dict[str, tuple[str, str, str]] = {
        ...
        "fast": ("How fast?", "medium", "Specify performance requirements or benchmarks"),
        "good": (
            "Define quality metrics",
            "medium",
            "Specify what makes a solution 'good' in this context",
        ),
        "better": ("Better than what?", "high", "Define the baseline or comparison point"),
        "easy": ("Easy for whom?", "medium", "Specify the target audience and their skill level"),
        "complex": (
    ...
    REQUIRED_ELEMENTS: list[tuple[str, list[str], str]] = [
        (
            "Context",
            ["background", "context", "situation", "problem", "goal"],
            "Add background information about the task or problem",
        ),
        (
            "Audience/Role",
            ["as a", "as if i am", "like i'm", "for a", "target audience"],
            "Specify who the output is for or what role to assume",
        ),
        (
            "Output Format",
            ["output", "format", "return", "in the form of"],
            "Specify the desired format (text, code, list, table, etc.)",
        ),
        (
            "Constraints",
            ["must", "should", "must not", "avoid", "requirement", "constraint"],
            "Add constraints or requirements that must be met",
        ),
        (
            "Purpose",
            ["why", "so that", "in order to", "purpose", "goal"],
    ...
    def analyze(self, prompt: str, current_profile: str | None = None) -> PromptAnalysis:
        """
        ...
        # Detect type
        detected_type, _confidence = self._detect_type(prompt_lower)
        ...
        # Calculate score
        score = self._calculate_score(word_count, missing, smells, detected_type, recommendations)
    ...
    def _detect_type(self, prompt: str) -> tuple[str, float]:
        ...
        max_possible = max(
            len(p.split()) for _, (patterns, _, _) in self.TYPE_PATTERNS.items() for p in patterns
        )
    ...
    def _detect_smells(self, prompt: str) -> list[PromptSmell]:
        ...
        for term, (explanation, severity, suggestion) in self.AMBIGUOUS_TERMS.items():
            ...
            if re.search(rf"\b{re.escape(term)}\b", prompt_lower):
                smells.append(
                    PromptSmell(
                        term=term, explanation=explanation, severity=severity, suggestion=suggestion
                    )
                )
    ...
    def _generate_challenges(self, prompt: str, prompt_lower: str) -> list[PromptChallenge]:
        ...
        def has_any(terms: list[str]) -> str | None:
            for t in terms:
    ...
    def _get_type_specific_recommendations(
        self, detected_type: str, prompt: str
    ) -> list[PromptRecommendation]:
    ...
    def _recommend_profile(self, detected_type: str, current_profile: str | None) -> str:
