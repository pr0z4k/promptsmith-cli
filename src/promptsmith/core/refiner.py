...
class PromptRefiner:
    ...
    def _load_profile(self, profile_name: str) -> RefinementProfile:
        # ProfileManager returns raw dict[str, Any] parsed from YAML (which
        # may carry extra bookkeeping keys like "name" alongside the fields
        # RefinementProfile declares). The cast asserts the shared contract
        # at this boundary rather than narrowing ProfileManager's broader
        # return type, which other callers rely on.
        try:
            return cast(RefinementProfile, self.profile_manager.get_profile(profile_name))
        except ProfileNotFoundError:
            profile = self.profile_manager.get_config(profile_name)
            if not profile:
                raise ProfileNotFoundError(profile_name) from None
            return cast(RefinementProfile, profile)

    def _expand_template(self, prompt: str, template_name: str | None) -> str:
    ...
    def refine(
        ...
        if result is None:
            ...
            self.last_backend_used = (
                "rule" if isinstance(instance, RuleBasedBackend) else backend_name
            )
