...
class YAMLConfigStore(ABC):  # noqa: B024 - marks "don't instantiate directly" by convention
    def __init__(
        self,
        config_dir: Path,
    ...
    def _validate_config(self, config_data: dict, source: str) -> dict:
        ...
        try:
            if hasattr(self.config_schema, "validate"):
                validated: dict = self.config_schema.validate(config_data)
                return validated
            return config_data
    ...
    def get_config(self, name: str) -> dict:
        # No filesystem path is built from `name` here - this is a pure
        # cache lookup, so an invalid name (e.g. a display name with a
        # space) is simply a miss, not a security concern. The
        # path-traversal guard belongs on add_config/delete_config, which
        # actually interpolate `name` into a file path.
