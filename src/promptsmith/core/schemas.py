...
class ProfileSchema:
    ...
    def validate(cls, data: dict[str, Any], source: str = "unknown") -> dict[str, Any]:
        ...
        if missing:
            raise ValueError(
                f"Profile in {source} is missing required fields: {', '.join(missing)}"
            )
        errors = []
...
class TemplateSchema:
    ...
    def validate(cls, data: dict[str, Any], source: str = "unknown") -> dict[str, Any]:
        if not isinstance(data, dict):
            raise ValueError(
                f"Template in {source} must be a dictionary, got {type(data).__name__}"
            )
        missing = [f for f in cls.REQUIRED_FIELDS if f not in data]
        if missing:
            raise ValueError(
                f"Template in {source} is missing required fields: {', '.join(missing)}"
            )
