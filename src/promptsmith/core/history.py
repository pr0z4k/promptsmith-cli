...
class HistoryStore:
    ...
    def _quarantine_corrupt_database(self) -> Path:
        ...
        counter = 1
        while target.exists():
            target = self.db_path.with_name(f"{self.db_path.name}.corrupt-{stamp}-{counter}")
            counter += 1
    ...
    def add(self, entry: HistoryEntry) -> int | None:
    ...
    def list(self, limit: int | None = None, offset: int = 0) -> list[HistoryEntry]:
        if not self._available:
            return []
