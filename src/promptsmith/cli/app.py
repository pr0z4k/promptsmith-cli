...
def _styled_button(label: str, **kwargs: Any) -> "Button":
...
from promptsmith.core.prompt_analyzer import PromptAnalysis, PromptAnalyzer
...
class PromptSmithApp(App):
    ...
    _refining_timer: Timer | None = None
    ...
    current_analysis: PromptAnalysis | None = None
    ...
    def __init__(self) -> None:
        ...
        try:
            ...
            self.profile_manager = ProfileManager(_PROFILES_DIR, user_dir=_USER_PROFILES_DIR)
            self.status_message = "Warning: No profiles loaded"
        ...
        try:
            ...
            self.status_message = "Warning: Analyzer not available"
        ...
        self.current_analysis: PromptAnalysis | None = None
    ...
    def _init_select_values(self) -> None:
        profile_options = [
            opt[1] for opt in self._profile_select._options if not _is_blank_select_value(opt[1])
        ]
        template_options = [
            opt[1] for opt in self._template_select._options if not _is_blank_select_value(opt[1])
        ]
        ...
        if self._valid_profile in profile_options:
            ...
            self.current_profile = str(profile_options[0])
    ...
    def on_button_pressed(self, event: Button.Pressed) -> None:
        ...
        action_fn = actions.get(event.button.id or "")
        if action_fn:
            try:
                action_fn()
            except Exception as e:
    ...
    def on_select_changed(self, event: Select.Changed) -> None:
        try:
            ...
            if event.select.id == "profile_select":
                ...
                self.current_template = None if event.value == "None" else str(event.value)
    ...
    def _get_profile_backend(self, profile_name: str) -> str:
        ...
        try:
            ...
            backend: str = profile.get("backend", "rule")
            return backend
        except Exception:
            return "rule"
    ...
    def _update_analysis_display(self) -> None:
        ...
        try:
            ...
            lines.append(
                f"[bold]{self.current_analysis.detected_type.upper()}[/bold] | Score: [bold]{score}/100[/bold]"
            )
            ...
            # Recommended profile
            lines.append(
                f"[cyan]Suggested Profile:[/cyan] {self.current_analysis.recommended_profile}"
            )
            ...
            if self.current_analysis.recommendations:
                ...
                lines.append("[magenta]Recommendations:[/magenta]")
            ...
            if self.current_analysis.challenges:
                ...
                lines.append("[bold cyan]Worth clarifying:[/bold cyan]")
    ...
    def action_analyze(self) -> None:
        ...
        try:
            ...
            self.current_analysis = self.analyzer.analyze(
                prompt, self.current_profile if self.current_profile != "None" else None
            )
    ...
    def _tick_refining_animation(self) -> None:
        try:
            ...
            label = (
                ...
                if self._refining_backend in ("llm", "hybrid")
                else "Refining"
            )
    ...
    def _refine_worker(self, prompt: str) -> None:
        try:
            ...
            if self.current_analysis is not None and self.current_analysis.is_ready:
                status = (
                    f"READY | Done (warning: {warning}) {backend_tag}"
                    if warning
                    else f"READY | Done. {backend_tag}"
                )
            else:
                status = (
                    f"Needs work | Done (warning: {warning}) {backend_tag}"
                    if warning
            ...
            self.call_from_thread(
                self._finish_refine_with_error, f"Error: Template not found - {e}"
            )
        except BackendError as e:
    ...
    def action_settings(self) -> None:
        if getattr(self, "_refine_in_progress", False):
            self.status_message = (
                "Please wait for the current refine to finish before opening Settings."
            )
            return
        try:
...
class HistoryScreen(ModalScreen):
    ...
    def __init__(self, main_app: "PromptSmithApp") -> None:
        ...
        # The caller (action_history) already verified history is available
        # before pushing this screen; the assert makes that precondition
        # explicit for the type checker instead of re-deriving it here.
        assert main_app.history is not None
        self.history: HistoryStore = main_app.history
    ...
    def _reload(self) -> None:
        ...
        self._set_status(
            f"{len(self._entries)} " + ("entry" if len(self._entries) == 1 else "entries")
        )
    ...
    def _clear_button(self) -> Button | None:
        try:
    ...
    def _clear_all(self) -> None:
        ...
        if not self._clear_armed:
            ...
            if btn is not None:
                warn = "[ Confirm Clear? ]"
                btn.label = Content(warn, spans=[Span(0, len(warn), style="bold #66FF66")])
            self._set_status(
...
class SettingsScreen(Screen):
    ...
    def action_download_custom_model(self) -> None:
        ...
        if not url:
            ...
            return
        self.main_app.status_message = "Downloading from URL in background..."
        self.app.pop_screen()
...
class ModelSwitchScreen(Screen):
    ...
    def action_apply_model(self) -> None:
        ...
        if value is None or value == self._AUTO:
            ...
            message = f"Model switched -> {Path(str(value)).name}"
...
class ProfileEditorScreen(Screen):
    ...
    def __init__(self, main_app: "PromptSmithApp") -> None:
        super().__init__()
        self.main_app = main_app
        self._loaded_id: str | None = None  # profile id currently populating the form
        self._loaded_was_builtin: bool = (
            False  # whether that id was a built-in (not yet overridden)
        )
        ...
        yield VerticalScroll(
            ...
            Select(
                [
                    (v, k)
                    for k, v in [
                        ("rule", "rule (deterministic)"),
                        ("llm", "llm (model-generated)"),
                        ("hybrid", "hybrid (rules + model polish)"),
                    ]
                ],
                id="backend_input",
                value="rule",
    ...
    def action_save(self) -> None:
        ...
        profile_id = (
            self._slugify(raw_id)
            if raw_id
            else self._slugify(self.query_one("#name_input", Input).value or role)
        )

        domain = [
            line.strip()
            for line in self.query_one("#domain_input", TextArea).text.splitlines()
            if line.strip()
        ]
        constraints = [
            line.strip()
            for line in self.query_one("#constraints_input", TextArea).text.splitlines()
            if line.strip()
        ]
        ...
        if self._loaded_id:
            try:
                version = self.main_app.profile_manager.get_profile(self._loaded_id).get(
                    "version", 1
                )
            except Exception:
                version = 1
        ...
        saved_path = (
            f"{user_dir}/{profile_id}.yaml" if user_dir is not None else f"{profile_id}.yaml"
        )

        if renamed:
            status.update(
                f"Saved as '{profile_id}' → {saved_path} (new copy — '{previous_id}' was left as-is)."
            )
        elif overwrote_builtin:
            status.update(
                f"Saved '{profile_id}' → {saved_path} (your copy — the built-in version ships untouched)."
            )
        else:
            status.update(f"Saved '{profile_id}' → {saved_path}.")
        self.query_one("#editor_hint", Static).update(
            f"Editing your custom profile '{profile_id}'."
        )
    ...
    def action_delete(self) -> None:
        ...
        if not self.main_app.profile_manager.is_user_defined(self._loaded_id):
            status.update(
                "Built-in profiles can't be deleted, only overridden by saving your own version."
            )
            return
        try:
