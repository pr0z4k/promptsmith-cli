...
def test_refine_falls_back_to_none_on_truncated_think_block(monkeypatch, tmp_path):
    ...
    assert result is None
    # The message is intentionally model-agnostic (other reasoning models
    # use different tag conventions than <think>), so it says "reasoning"
    # rather than naming the specific tag.
    assert "reasoning" in backend.last_error.lower()
