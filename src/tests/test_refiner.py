...
def refiner(tmp_path):
    ...
    (pdir / "test-profile.yaml").write_text(
        yaml.dump(
            {
                "name": "Test",
                "role": "Tester",
    ...
    (tdir / "test-template.yaml").write_text(
        yaml.dump(
            {
                "name": "Test Template",
...
def hybrid_refiner(tmp_path):
    ...
    (pdir / "hybrid-profile.yaml").write_text(
        yaml.dump(
            {
                "name": "Hybrid Test",
...
def test_hybrid_result_does_not_leak_raw_profile_framing(tmp_path, monkeypatch):
    ...
    (pdir / "adobe-experience-developer.yaml").write_text(
        yaml.dump(
            {
                ...
                "constraints": [
                    "Reference AEM best practices (e.g., component design, templating)."
                ],
                "vendor": "generic",
                "backend": "hybrid",
            }
