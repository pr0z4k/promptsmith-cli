...
from promptsmith.utils.path_utils import get_asset_path, get_project_root
...
class TestFrozenModeResolution:
    ...
    def test_project_root_uses_executable_directory_when_frozen(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(sys, "frozen", True, raising=False)
        monkeypatch.setattr(
            sys, "_MEIPASS", str(tmp_path / "some_other_meipass_dir"), raising=False
        )
...
class TestUserDataDirResolution:
    ...
    def test_user_data_dir_env_override_wins_regardless_of_frozen_state(
