...
from promptsmith.core.pipeline import IntentCompilationResult, IntentCompiler
...
class TestIntentCompiler:
    ...
    def test_compile_with_profile(self, compiler):
        ...
        result = compiler.compile("Explain Python", profile_name="general")
    ...
    def test_compile_with_template(self, compiler):
        ...
        result = compiler.compile(
            "Python tutorial", template_name=None  # No template available in test
        )
        assert result.refined_prompt is not None
...
def test_auto_recommended_profile_is_actually_applied(tmp_path):
    ...
    (profiles_dir / "react-developer.yaml").write_text(
        yaml.dump(
            {
    ...
    result = compiler.compile("Create a reusable React component with hooks for a login form")
